...
def test_golden_rf_binary(medium_classification):
    ...
    # Floor checks — Rust and sklearn RF produce different values
    assert m["accuracy"] > 0.6, f"accuracy={m['accuracy']} too low"
    assert m["roc_auc"] > 0.7, f"roc_auc={m['roc_auc']} too low"
    assert m["brier_score"] < 0.3, f"brier={m['brier_score']} too high"
...
def test_golden_logistic_binary(medium_classification):
    ...
    with warnings.catch_warnings():
        ...
        model = ml.fit(data=s.train, target="target", algorithm="logistic", seed=42)
    m = ml.evaluate(model, s.valid)
    assert m["accuracy"] > 0.75, f"accuracy={m['accuracy']} too low"
    assert m["roc_auc"] > 0.80, f"roc_auc={m['roc_auc']} too low"
    assert m["brier_score"] < 0.2, f"brier={m['brier_score']} too high"
...
def test_golden_linear_regression(medium_regression):
    ...
    m = ml.evaluate(model, s.valid)
    assert m["rmse"] < 0.8, f"rmse={m['rmse']} too high"
    assert m["r2"] > 0.90, f"r2={m['r2']} too low"
...
def test_golden_knn_binary(medium_classification):
    ...
    m = ml.evaluate(model, s.valid)
    # Rust KNN and sklearn KNN differ significantly (distance calc, tie-breaking)
    # Values updated 2026-04-03. Floor checks only.
    assert m["accuracy"] > 0.4, f"accuracy={m['accuracy']} too low"
    assert m["roc_auc"] > 0.4, f"roc_auc={m['roc_auc']} too low"
...
def test_golden_decision_tree_clf(medium_classification):
    ...
    m = ml.evaluate(model, s.valid)
    assert m["accuracy"] > 0.5, f"accuracy={m['accuracy']} too low"
    assert m["brier_score"] < 0.5, f"brier={m['brier_score']} too high"
...
def test_golden_naive_bayes_clf(medium_classification):
    ...
    m = ml.evaluate(model, s.valid)
    assert m["accuracy"] > 0.7, f"accuracy={m['accuracy']} too low"
    assert m["roc_auc"] > 0.7, f"roc_auc={m['roc_auc']} too low"
...
def test_golden_adaboost_clf(medium_classification):
    ...
    m = ml.evaluate(model, s.valid)
    assert m["accuracy"] > 0.6, f"accuracy={m['accuracy']} too low"
    assert m["roc_auc"] > 0.6, f"roc_auc={m['roc_auc']} too low"
