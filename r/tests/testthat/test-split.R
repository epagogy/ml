...
test_that("CV mode returns ml_split_result with folds and test", {
  cv <- ml_split(iris, "Species", seed = 42L, folds = 5L)
  expect_s3_class(cv, "ml_split_result")
  expect_equal(length(cv$folds), 5L)
  # Test partition held out (not on CV, on the split)
  expect_true(nrow(cv$test) > 0L)
  expect_true(nrow(cv$test) < nrow(iris))
})
...
test_that("CV split_result has test but train is dev data", {
  cv <- ml_split(iris, "Species", seed = 42L, folds = 3L)
  # $test is available (held out for assess)
  expect_true(nrow(cv$test) > 0L)
  # $train is dev data (used internally for folds)
  expect_true(nrow(cv$train) > 0L)
  # $folds present
...
test_that("time= with folds produces temporal CV with test holdout", {
  n <- 200L
  df <- data.frame(t = seq_len(n), x = stats::rnorm(n), y = stats::rnorm(n))
  cv <- ml_split(df, "y", seed = 42L, time = "t", folds = 3L)
  expect_s3_class(cv, "ml_split_result")
  expect_equal(length(cv$folds), 3L)
  # Test held out from end of time series
  expect_true(nrow(cv$test) > 0L)
...
test_that("groups= with folds produces GroupKFold with test holdout", {
  n <- 200L
  grp <- rep(paste0("g", seq_len(20L)), each = 10L)
  df  <- data.frame(group = grp, x = stats::rnorm(n), y = stats::rnorm(n))
  ...
  # Test groups held out
  expect_true(nrow(cv$test) > 0L)
  # No group leakage across train/valid in each fold
  dev_data <- cv$train  # dev data stored as train when folds present
  for (f in cv$folds) {
    train_grps <- unique(dev_data$group[f$train])
    valid_grps <- unique(dev_data$group[f$valid])
    expect_length(intersect(train_grps, valid_grps), 0L)
  }
})
