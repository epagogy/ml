...
test_that("evaluate rejects genuinely unsplit data in strict mode", {
  ml_config(guards = "strict")
  withr::defer(ml_config(guards = "off"))
  s     <- iris_split()
  model <- ml_fit(s$train, "Species", algorithm = "logistic", seed = 42L)
  ...
  unsplit <- iris[1:30, ]
  expect_error(ml_evaluate(model, unsplit), class = "partition_error")
})

test_that("evaluate allows unsplit data with guards off", {
  ml_config(guards = "off")
  s     <- iris_split()
  model <- ml_fit(s$train, "Species", algorithm = "logistic", seed = 42L)
  unsplit <- iris[1:30, ]
  result <- ml_evaluate(model, unsplit)
  expect_s3_class(result, "ml_metrics")
})
