...
test_that("assess rejects genuinely unsplit data in strict mode", {
  ml_config(guards = "strict")
  withr::defer(ml_config(guards = "off"))
  s     <- iris_split()
  model <- ml_fit(s$train, "Species", algorithm = "logistic", seed = 42L)
  # Use data that never passed through split (not a copy of split output)
  unsplit <- iris[1:30, ]
  expect_error(ml_assess(model, test = unsplit), class = "partition_error")
})

test_that("assess allows unsplit data with guards off", {
