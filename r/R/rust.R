...
#' Deterministic shuffle using Rust PCG-XSH-RR.
#' Returns 1-based indices (R convention). Falls back to R's sample() if
#' Rust backend is not available.
#' @keywords internal
.ml_shuffle <- function(n, seed) {
  # Validate seed: non-negative, numeric, non-NA
  if (!is.numeric(seed) || is.na(seed)) {
    stop("seed must be a non-NA numeric value", call. = FALSE)
  }
  if (seed < 0) {
    stop("seed must be non-negative, got ", seed, call. = FALSE)
  }
  # Pass seed as double to Rust (extendr binding takes i64, double preserves
  # integer values up to 2^53 which covers the practical seed range).
  # No modulo reduction -- seeds pass through to Rust PCG unchanged.
  seed_dbl <- as.double(seed)
  result <- tryCatch(
    .Call(wrap__ml_rust_shuffle, as.integer(n), seed_dbl),
    error = function(e) {
      # Warn on fallback instead of silent degradation
      cli::cli_warn("Rust backend unavailable for shuffle -- falling back to R's RNG. Cross-language parity not guaranteed.")
      NULL
    }
  )
  if (!is.null(result)) return(result)
  # Fallback: R's own RNG (same correctness, different permutation)
  withr::local_seed(as.integer(seed_dbl %% .Machine$integer.max))
  sample.int(n)
}

#' Canonical partition sizes: c(n_train, n_valid, n_test).
#' Uses round(n * ratio) -- matches Python.
#' @keywords internal
.ml_partition_sizes <- function(n, ratio = c(0.6, 0.2, 0.2)) {
  result <- tryCatch(
    .Call(wrap__ml_rust_partition_sizes,
          as.integer(n), ratio[1], ratio[2], ratio[3]),
    error = function(e) NULL
  )
  if (!is.null(result)) return(result)
  # Fallback: pure R (identical formula -- banker's rounding matches Python)
  n_train <- round(n * ratio[1])
  n_valid <- round(n * ratio[2])
  n_test  <- n - n_train - n_valid
  as.integer(c(n_train, n_valid, n_test))
}
...
.is_rust_engine <- function(engine) {
  is.list(engine) && is.character(engine$type) && startsWith(engine$type, "rust_")
}
