...
.fit_engine <- function(X, y, task, seed, algorithm, engine = "auto", ...) {
  ...
  switch(algorithm,
    ...
    extra_trees       = .fit_extra_trees_cran(X, y, task, seed, dots, balance_weights),
...
.predict_engine <- function(engine, X, task, algorithm) {
  ...
  switch(algorithm,
    ...
    extra_trees   = .predict_ranger(engine, X, task),
...
.predict_proba_engine <- function(engine, X, task, algorithm) {
  ...
  switch(algorithm,
    ...
    extra_trees   = .proba_ranger(engine, X),
...
.fit_ranger <- function(X, y, task, seed, dots, balance_weights = NULL) {
  ...
  if (task == "classification") {
    # classification: sqrt(p) features per split (ranger default, matches sklearn)
    ...
    # regression: use all features per split (matches sklearn RandomForestRegressor
    # default max_features=1.0). Ranger's default floor(p/3) is too aggressive for
    # datasets with few features, causing large parity gaps vs Python.
    mtry <- dots[["mtry"]] %||% ncol(X)
    ranger::ranger(
      x = X, y = as.numeric(y),
      num.trees = dots[["num.trees"]],
      mtry = mtry,
...
# ── Extra Trees CRAN fallback (ranger splitrule="extratrees") ─────────────────

.fit_extra_trees_cran <- function(X, y, task, seed, dots, balance_weights = NULL) {
  if (!requireNamespace("ranger", quietly = TRUE)) {
    config_error("'ranger' required for extra_trees. Install with: install.packages('ranger')")
  }
  dots[["num.trees"]] <- dots[["num.trees"]] %||% 500L
  if (task == "classification") {
    y_factor <- if (is.factor(y)) y else factor(y)
