...
test_that("validate rejects untagged data in strict mode", {
  ml_config(guards = "strict")
  withr::defer(ml_config(guards = "off"))
  s     <- iris_split()
  model <- ml_fit(s$train, "Species", algorithm = "logistic", seed = 42L)
  ...
  expect_error(
    ml_validate(model, test = untagged, rules = list(accuracy = ">0.0")),
    class = "partition_error"
  )
})

test_that("validate allows untagged data with guards off", {
  ml_config(guards = "off")
  s     <- iris_split()
  model <- ml_fit(s$train, "Species", algorithm = "logistic", seed = 42L)
  untagged <- data.frame(s$test)
  gate <- ml_validate(model, test = untagged, rules = list(accuracy = ">0.0"))
  expect_s3_class(gate, "ml_validate_result")
})
