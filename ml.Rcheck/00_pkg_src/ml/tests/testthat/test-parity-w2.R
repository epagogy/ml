...
test_that("W2-07: CV split result has correct S3 class (ml_cv_result)", {
  cv <- ml_split(iris, "Species", seed = 42L, folds = 3L)
  expect_true(inherits(cv, "ml_cv_result"),
              info = paste("Expected ml_cv_result, got:", paste(class(cv), collapse=", ")))
})
