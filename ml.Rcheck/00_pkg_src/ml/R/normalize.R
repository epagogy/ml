...
#' @returns A string: `"classification"` or `"regression"`.
#' @keywords internal
...
#' @returns A named list (NormState) containing encoding/scaling state for use
#'   with `.transform()` and `.encode_target()`.
#' @keywords internal
...
#' @returns A list with `$X` (encoded data.frame) and `$norm` (updated NormState
#'   with imputation/scaling parameters filled in).
#' @keywords internal
.transform_fit <- function(X, norm) {
  ...
  if (ncol(num_X) > 0) {
    num_mat <- as.matrix(num_X)
    inf_mask <- is.infinite(num_mat)
    if (any(inf_mask)) {
      data_error("Infinite values (Inf/-Inf) found in features. Replace with finite values before training.")
    }
    nan_mask <- is.nan(num_mat)
    if (any(nan_mask)) {
      n_nan <- sum(nan_mask)
      cli::cli_warn(paste0(n_nan, " NaN value(s) in features -- coerced to NA for imputation."))
      for (col in names(num_X)) {
        nan_idx <- is.nan(X[[col]])
        if (any(nan_idx)) X[[col]][nan_idx] <- NA
      }
    }
  }
...
#' @returns An encoded `data.frame` with columns aligned to training features.
#' @keywords internal
...
#' @returns An integer vector of 0-based encoded labels.
#' @keywords internal
...
#' @returns A character vector of decoded class labels, or the original
#'   predictions if no label encoding was used.
#' @keywords internal
...
.apply_ordinal <- function(X, category_maps) {
  ...
  for (col in names(category_maps)) {
    ...
    # 3-step ordinal (Python delta P1 #6):
...
.apply_onehot_fit <- function(X, onehot_cols, onehot_levels) {
  ...
  for (col in onehot_cols) {
    ...
    for (lvl in lvls) {
      ...
      # Collision detection (Python delta P1 #10)
