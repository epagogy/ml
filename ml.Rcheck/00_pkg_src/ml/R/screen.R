...
#'
#' @param data An `ml_split_result` or `ml_cv_result` (NOT a raw data.frame --
...
.screen_impl <- function(data, target, algorithms = NULL, seed = NULL,
  ...
  if (!inherits(data, c("ml_split_result", "ml_cv_result"))) {
    config_error(paste0(
  ...
  src  <- if (inherits(data, "ml_cv_result")) data[["data"]] else data[["train"]]
  ...
  if (inherits(data, "ml_split_result")) {
    ...
    train_data <- data[["data"]][data[["folds"]][[1]]$train, , drop = FALSE]
    eval_data  <- data[["data"]][data[["folds"]][[1]]$valid, , drop = FALSE]
  }
  ...
  for (algo in algos) {
    ...
    result <- tryCatch({
      ...
      metrics <- .evaluate_impl(model, eval_data, .guard = FALSE)
