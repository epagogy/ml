...
ml_check <- function(data, target, algorithm = "random_forest", seed) {
  ...
  if (!identical(p1, p2)) {
    model_error(paste0(
      ...
      "Check that seed is set correctly and the algorithm supports determinism."
    ))
  }
  ...
  if (task == "classification") {
    ...
    if (n_unique <= 1L) {
      ...
      warns <- c(warns, paste0(
        ...
        "ml_fit() will raise an error. Need at least 2 classes."
      ))
    } else {
...
#' Print a data check report
#'
#' @param x An `ml_check_report` object.
