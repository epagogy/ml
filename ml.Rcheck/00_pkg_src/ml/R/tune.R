...
#'
#' @param data A data.frame or `ml_cv_result`
...
.tune_impl <- function(data, target, model = NULL, algorithm = NULL,
  ...
  # Coerce data
  data <- if (inherits(data, "ml_cv_result")) data[["data"]] else .coerce_data(data)
  ...
  # NaN pre-check (Python delta P1 #11)
  ...
  if (method == "grid") {
    ...
    # Random search -- reset to same seed for deterministic params
    set.seed(seed)
