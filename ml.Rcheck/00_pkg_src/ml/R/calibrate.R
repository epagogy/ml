...
#' Predict from a calibrated model
#'
#' Applies Platt-calibrated probabilities from the base model, then returns
#' either class labels or calibrated probability estimates.
#'
#' @param object An `ml_calibrated_model` from [ml_calibrate()].
#' @param newdata A data.frame with the same features used for training.
#' @param proba Logical. If TRUE, return calibrated class probabilities.
#' @param ... Ignored.
#' @returns A character vector of predicted class labels, or (if `proba = TRUE`)
#'   a data.frame with one column per class containing calibrated probabilities.
#' @export
...
#' Print a calibrated model
#'
#' @param x An `ml_calibrated_model` object.
