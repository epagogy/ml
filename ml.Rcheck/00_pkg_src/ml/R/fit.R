...
#'
#' Trains a model using cross-validation (if `data` is an `ml_cv_result`) or
#' holdout (if `data` is a `data.frame`). Automatically detects task type,
...
#' ordinal encoding.
#'
#' @param data A `data.frame` or `ml_cv_result` (for cross-validation)
#' @param target Target column name (string)
#' @param algorithm "auto" (default), "logistic", "decision_tree",
#'   "random_forest", "extra_trees", "gradient_boosting", "xgboost",
#'   "linear", "elastic_net", "svm", "knn", "naive_bayes", "adaboost",
#'   "histgradient". See [ml_algorithms()] for availability.
...
.fit_impl <- function(data, target, algorithm = "auto", seed = NULL,
  ...
  if (detected_task == "classification") {
    ...
    if (n_classes < 2L) {
      data_error(paste0(
        "Target '", target, "' has only 1 unique value (",
        unique(y[!is.na(y)])[[1]], "). Need at least 2 classes."
      ))
    }
  }
...
.roc_auc_binary <- function(y_true, y_score) {
  ...
  fpr <- fp / n_neg
  # Trapezoid rule
  sum(diff(c(0, fpr)) * (c(0, tpr) + c(tpr, 0)[-1]) / 2)
}
